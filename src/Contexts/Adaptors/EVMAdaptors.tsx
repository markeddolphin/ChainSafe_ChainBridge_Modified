...
import bridgeContractAbi from "../../bridgeContract.json";
...
export const EVMHomeAdaptorProvider = ({
  ...
  useEffect(() => {
    if (network) {
      console.log("currently connected network: => ", network);
              ...
              .then((success) => {
                if (success) {
                  if (homeChainConfig && network && isReady && provider) {
                    ...
                    console.log("home chain config: => ", homeChainConfig);
  ...
  const deposit = useCallback(
    async (
      ...
      console.log("JsonRPCSigner: => ", signer);
      ...
      console.log("erc20 contract instance: => ", erc20);
      ...
      try {
        ...
        if (allowance.lt(BigNumber.from(utils.parseUnits("100", erc20Decimals)))) {
          await (
            await erc20.approve(
              (homeChainConfig as EvmBridgeConfig).bridgeAddress,
              BigNumber.from(utils.parseUnits("100", erc20Decimals)),
              {
                gasPrice: BigNumber.from(
                  utils.parseUnits(
                    (
        ...
        console.log("allowance for erc20 handler: => ", allowance.toString());
        // const estimateGas = await homeBridge.estimateGas.deposit(destinationChainId, token.resourceId, data, {});
        // console.log("Estimated Gas: => ", estimateGas);

        const bridgeContract = new ethers.Contract((homeChainConfig as EvmBridgeConfig).bridgeAddress, bridgeContractAbi.abi as any, signer);

        console.log("Bridge contract instance: => ", bridgeContract);
        await (
          await bridgeContract.deposit(destinationChainId, token.resourceId, data, {
            gasPrice: utils.parseUnits(((homeChainConfig as EvmBridgeConfig).defaultGasPrice || 200).toString(), 9) as any,
            gasLimit: utils.parseUnits('21', 4) as any,
            value: utils.parseUnits((bridgeFee || 0).toString(), 18) as any,
          })
        ).wait();
  ...
  const wrapToken = async (value: number): Promise<string> => {
    ...
    try {
      const tx = await wrapper.deposit({
        value: parseUnits(`${value}`, homeChainConfig.decimals) as any,
        gasPrice: BigNumber.from(
          utils.parseUnits(
            (
...
export const EVMDestinationAdaptorProvider = ({
  ...
  useEffect(() => {
    ...
    if (destinationChainConfig?.rpcUrl.startsWith("wss")) {
      ...
      //   );
      // }
      provider = new ethers.providers.WebSocketProvider(destinationChainConfig.rpcUrl, destinationChainConfig.networkId);

      console.log("Substrate provider: => ", provider);
    } else {
